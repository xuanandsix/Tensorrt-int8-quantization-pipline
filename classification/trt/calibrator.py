...
class ResNetEntropyCalibrator(trt.IInt8EntropyCalibrator2):
    def __init__(self, training_data, cache_file, batch_size=8):
        ...
        # self.data = self.load_totaltext_data(training_data)
        # self.data_path = self.load_totaltext_data_path(training_data)
        self.dataset_test = Imagenet1k(training_data)

        self.batch_size = batch_size
        self.current_index = 0

        # Allocate enough memory for a whole batch.
        test_np = np.zeros((1, 3, 224, 224), dtype=np.float32)
        # self.device_input = cuda.mem_alloc(self.data[0].nbytes * self.batch_size)
        self.device_input = cuda.mem_alloc(test_np[0].nbytes * self.batch_size)
    ...
    def get_batch(self, names):
        ...
        if self.current_index + self.batch_size > len(self.dataset_test):
            return None
